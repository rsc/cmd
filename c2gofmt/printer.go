...
func (p *Printer) Print(args ...interface{}) {
	for _, arg := range args {
		...
		case *cc.Init:
			p.printInit(nil, arg)
		case typedInit:
			if arg.typ.Kind == cc.Array && arg.typ.Width == nil && len(arg.init.Braced) > 0 {
				typ := *arg.typ
				arg.typ = &typ
				arg.typ.Width = &cc.Expr{Op: cc.Name, Text: fmt.Sprint(len(arg.init.Braced))}
			}
			p.printInit(arg.typ, arg.init)
		case *cc.Prog:
			p.printProg(arg)
		case *cc.Stmt:
			p.printStmt(arg, true)
		case unnestBlock:
			p.printStmt(arg.stmt, false)
		case []*cc.Stmt:
			for _, s := range arg {
				p.Print(s)
			}
		...
		case cc.Comment:
			...
			// p.Print(fmt.Sprintf("/*cstart:%d.%d*/", com.Span.Start.Line, com.Span.Start.Byte))
			// defer p.Print(fmt.Sprintf("/*cend:%d.%d*/", com.Span.End.Line, com.Span.End.Byte))
			if com.Suffix {
		...
		case PrintSpecial:
			...
			case Unnewline:
				buf := p.buf.Bytes()
				if len(buf) < 1+p.indent || buf[len(buf)-1-p.indent] != '\n' {
					goto KeepNewline
				}
				for i := 0; i < p.indent; i++ {
					if buf[len(buf)-1-i] != '\t' {
				...
				p.buf.Truncate(len(buf) - (1 + p.indent))
			KeepNewline:
			}
		}
	}
...
func (p *Printer) printExpr(x *cc.Expr, prec int) {
	...
	case SideEffectFunc:
		...
		if len(x.After) == 0 {
			p.Print(sep, "return ", x.Left)
		} else {
			p.Print(sep, "_r := ", x.Left)
			for _, stmt := range x.After {
				p.Print("; ", stmt)
			}
			p.Print("; return _r")
		}
	...
	case cc.Call:
		left := x.Left
		if left.Op == cc.Paren && left.Left.Op == cc.Indir {
			// print (*f)() as f()
			left = left.Left.Left
		}
		p.Print(exprPrec{left, precAddr}, "(")
	...
	case cc.Number:
		p.Print(strings.TrimPrefix(strings.TrimRight(x.Text, "LlUu"), "L"))
	...
	case cc.String:
		for i, str := range x.Texts {
			...
			p.Print(strings.TrimPrefix(str, "L"))
		}
...
func (p *Printer) printInit(typ *cc.Type, x *cc.Init) {
	...
	for i, y := range x.Braced {
		...
		p.Print(",")
		if i+1 < len(x.Braced) && y.Span.End.Line != x.Braced[i+1].Span.Start.Line {
			p.Print(Newline)
			startLine = true
		}
	}
...
func (p *Printer) printStmt(x *cc.Stmt, canNest bool) {
	printedComments := false
	if x.Op == BlockNoBrace {
		canNest = false
	}
	if len(x.Labels) > 0 {
...
func (p *Printer) printDecl(decl *cc.Decl, top bool) {
	// p.Print(fmt.Sprintf("/*before:%d.%d*/", decl.Span.Start.Line, decl.Span.Start.Byte))
	// defer p.Print(fmt.Sprintf("/*after:%d.%d*/", decl.Span.End.Line, decl.Span.End.Byte))

	p.Print(decl.Comments.Before)
	defer p.Print(decl.Comments.Suffix, decl.Comments.After)

	if decl.Blank {
		// p.Print(fmt.Sprintf("/*blank:%d.%d*/", decl.Span.Start.Line, decl.Span.Start.Byte))
		return
	}

	if decl.Storage&cc.Extern != 0 {
		p.Print("/* extern ")
		defer p.Print(" */")
	}
	...
	if t != nil && t.Kind == cc.Func {
		p.printFuncDecl(decl)
		return
	}

	if t != nil && decl.Init != nil && len(decl.Init.Braced) > 0 {
		if top {
			p.Print("var ", decl.Name, " = ", typedInit{decl.Type, decl.Init})
		} else {
			p.Print(decl.Name, " := ", typedInit{decl.Type, decl.Init})
		}
		return
	}

	if decl.Init != nil && !top {
		p.Print(decl.Name, " := ", decl.Init)
		return
	}
...
func (p *Printer) printFuncDecl(decl *cc.Decl) {
	...
	p.Print(" {", Indent, unnestBlock{decl.Body}, Unindent, Newline, "}", Newline)
}
