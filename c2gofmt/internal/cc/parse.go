...
func Read(name string, r io.Reader, types []string) (*Prog, error) {
	return ReadMany([]string{name}, []io.Reader{r}, types)
}

func ReadMany(names []string, readers []io.Reader, types []string) (*Prog, error) {
	lx := &lexer{byte: 1e6}
	lx.typeNames = make(map[string]bool)
	for _, typ := range types {
		lx.typeNames[typ] = true
	}
	var prog *Prog
