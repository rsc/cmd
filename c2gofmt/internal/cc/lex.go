...
Restart:
	...
	if isspace(c) {
		i := 1
		for i < len(in) && isspace(in[i]) && in[i] != '\n' {
			i++
		}
		if in[0] == '\n' && i < len(in) && in[i] == '\n' {
			lx.skip(1)
	...
	if isalpha(c) {
		...
		if yy.decl == nil && lx.typeNames[lx.tok] {
			yy.typ = &Type{Kind: TypedefType, Name: yy.str, Base: IntType}
			return tokTypeName
		}
		if lx.tok == "EXTERN" {
			goto Restart
...
func (lx *lexer) assignComments() {
	...
	for _, x := range lx.pre {
		...
		xcom := x.GetComments()
		for len(line) > 0 && line[0].Start.Byte <= start.Byte {
